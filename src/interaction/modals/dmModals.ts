...
export async function handleDmModals(interaction: ModalSubmitInteraction): Promise<boolean> {
  ...
  if (
    ...
    if (!parsed) {
      return interaction.reply({ content: "That share request expired.", ephemeral: true }).then(() => true).catch(() => true);
    }
    ...
    const detailKind: any = base === CustomIds.DmModalSteam ? "STEAM" : base === CustomIds.DmModalServerName ? "SERVER_NAME" : "SERVER_IP";

    const value = interaction.fields.getTextInputValue("value")?.trim() ?? "";
    const valid = dmShareFlowService.validateDetail(detailKind, value);

    if (!valid.ok) {
      return interaction.reply({ content: `❌ ${valid.message ?? "Invalid value."}`, ephemeral: true }).then(() => true).catch(() => true);
    }

    const share: any = { guildId, userId, gameId, gameName, detailKind, detailValue: value };
    ...
    return interaction.reply({
      ...
      components: [
        new ActionRowBuilder<ButtonBuilder>().addComponents(
          new ButtonBuilder().setCustomId(dmShareFlowService.dmId(CustomIds.DmConfirmPost, share)).setLabel("Confirm").setStyle(ButtonStyle.Primary),
          new ButtonBuilder().setCustomId(dmShareFlowService.dmId(CustomIds.DmCancelPost, share)).setLabel("Cancel").setStyle(ButtonStyle.Secondary),
        ),
      ],
