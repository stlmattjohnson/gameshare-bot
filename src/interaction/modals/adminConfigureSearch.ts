...
export async function handleAdminConfigureSearch(interaction: ModalSubmitInteraction): Promise<boolean> {
  const { base, key } = parseSessionCustomId(interaction.customId);

  if (base === CustomIds.AdminConfigureSearch) {
    if (!key) return interaction.reply(expiredMessage("/gameshare admin configure-games")).then(() => true).catch(() => true);

    const state = adminUxStore.get(key);
    if (!state) return interaction.reply(expiredMessage("/gameshare admin configure-games")).then(() => true).catch(() => true);
    ...
    if (!next) return interaction.reply(expiredMessage("/gameshare admin configure-games")).then(() => true).catch(() => true);

    const ui = await renderAdminConfigure(key, next);
    return interaction.reply({ ...(ui as any), ephemeral: true }).then(() => true).catch(() => true);
  }
