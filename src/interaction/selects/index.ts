...
export async function handleSelectInteraction(interaction: StringSelectMenuInteraction): Promise<boolean | InteractionResponse<boolean>> {
  const { base } = parseSessionCustomId(interaction.customId);

  if (base === CustomIds.DmDetailPick) return handleDmDetailPick(interaction);
  if (base === CustomIds.AdminConfigureToggleSelect) return handleAdminConfigureToggle(interaction);
  if (base === CustomIds.UserRolesPickSelect) return handleUserRolesPick(interaction);
