...
import { userRolesUxStore, renderUserRoles } from "../../services/ux/userRolesUx.ts";
...
export async function handleUserRolesPick(interaction: StringSelectMenuInteraction): Promise<boolean> {
  const { base, key } = (() => {
  ...
  if (!key) return interaction.reply({ content: "State expired. Run /gameshare roles", ephemeral: true }).then(() => true).catch(() => true);

  const state = userRolesUxStore.get(key);
  if (!state) return interaction.reply({ content: "State expired. Run /gameshare roles", ephemeral: true }).then(() => true).catch(() => true);
  userRolesUxStore.touch(key);

  const enabledIds = await guildConfigService.listEnabledGameIds(state.guildId);
  const enabledSet = new Set(enabledIds);

  const current = new Set(await userGameRolePrefRepo.listSelectedGameIds(state.guildId, state.userId));

  const enabledGames = enabledIds.map((id) => gameCatalog.getById(id)).filter(Boolean as any);
  ...
  await userGameRolePrefRepo.setSelectedGameIds(state.guildId, state.userId, Array.from(current));
  ...
  if (interaction.inGuild() && interaction.guild) {
    ...
    if (member) {
      for (const gameId of enabledIds) {
        const roleId = await guildConfigService.getRoleId(state.guildId, gameId);
        if (!roleId) continue;
  ...
  if (!refreshed) return interaction.reply({ content: "State expired. Run /gameshare roles", ephemeral: true }).then(() => true).catch(() => true);
  return interaction.update(await renderUserRoles(key, refreshed)).then(() => true).catch(() => true);
}
