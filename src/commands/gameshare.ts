...
import { createAdminSession, renderAdminConfigure } from "../services/ux/adminConfigureGamesUx.js";
import { createUserRolesSession, renderUserRoles } from "../services/ux/userRolesUx.js";
...
export const gameshareCommand = new SlashCommandBuilder()
  ...
  .addSubcommandGroup((g) =>
    g
      ...
      .addSubcommand((s) =>
        s.setName("configure-games").setDescription("Enable/disable recognized games")
      )
      .addSubcommand((s) => s.setName("status").setDescription("Show config + health status"))
  )
  .addSubcommand((s) => s.setName("opt-in").setDescription("Opt in to presence-based game share prompts"))
  .addSubcommand((s) => s.setName("opt-out").setDescription("Opt out and stop DMs"))
  .addSubcommand((s) => s.setName("roles").setDescription("Pick which game roles you want"))
  .addSubcommand((s) => s.setName("privacy").setDescription("Privacy details"))
  .addSubcommand((s) => s.setName("delete-my-data").setDescription("Delete your saved data for this server"))
  .setDMPermission(false);

export async function handleGameshare(interaction: ChatInputCommandInteraction) {
  if (!interaction.inGuild() || !interaction.guildId) {
    return safeEphemeralReply(interaction, { content: "This command can only be used in a server." });
  }
  ...
  if (subgroup === "admin") {
    ...
    const isAdmin =
      member?.has(PermissionFlagsBits.ManageGuild) || member?.has(PermissionFlagsBits.Administrator);
    ...
    if (sub === "set-channel") {
      ...
      if (ch.type !== ChannelType.GuildText) {
        return safeEphemeralReply(interaction, { content: "Please select a text channel." });
      }

      const me = interaction.guild?.members.me;
      if (!me) return safeEphemeralReply(interaction, { content: "Bot member not found." });

      const perms = (ch as TextChannel).permissionsFor(me);
      if (!perms?.has(["ViewChannel", "SendMessages"])) {
        return safeEphemeralReply(interaction, {
          content: "I can't post in that channel. Grant **View Channel** + **Send Messages**."
        });
      }

      await guildConfigService.setAnnounceChannel(interaction.guildId, ch.id);
      return safeEphemeralReply(interaction, { content: `✅ Announce channel set to <#${ch.id}>` });
    }
    ...
    if (sub === "status") {
      ...
      const enabledIds = await guildConfigService.listEnabledGameIds(interaction.guildId);
      const mappings = await guildConfigService.listMappings(interaction.guildId);

      const enabledNames = enabledIds
        .map((id) => gameCatalog.getById(id)?.name ?? id)
        .slice(0, 25);
      ...
      return safeEphemeralReply(interaction, {
        content: [
          ...
          "",
          enabledNames.length ? `**Some enabled games:** ${enabledNames.join(", ")}` : "",
          missingRoles.length ? `**Missing mapped roles:** ${missingRoles.map((m) => m.gameId).join(", ")}` : "",
          "",
          presenceIntentHint
        ].filter(Boolean).join("\n")
      });
    ...
    return safeEphemeralReply(interaction, { content: "Unknown admin subcommand." });
  }
  ...
  if (sub === "opt-out") {
    await optInService.setOptIn(interaction.guildId, interaction.user.id, false);
    return safeEphemeralReply(interaction, { content: "✅ You’re opted out. I won’t DM you or monitor your presence." });
  }

  if (sub === "roles") {
    const opted = await optInService.isOptedIn(interaction.guildId, interaction.user.id);
    if (!opted) {
      return safeEphemeralReply(interaction, { content: "You must `/gameshare opt-in` first." });
    }
    const state = { guildId: interaction.guildId, userId: interaction.user.id, page: 0 };
  ...
  if (sub === "delete-my-data") {
    ...
    return safeEphemeralReply(interaction, { content: "✅ Deleted your saved data for this server." });
  }
